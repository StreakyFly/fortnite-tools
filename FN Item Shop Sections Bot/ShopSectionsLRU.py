...
def main():
    try:
        ...
        if sections_data != cache1:  # or True:  # todo remove "or True" once done testing!!
                ...
                for b in get_data("all_data"):
                    ...
                    if name == section_id:
                        ...
                        try:
                            ...
                            print("HAS_sectionDisplayName:", name)
                        except:
                            name = a
                            print("DOES_NOT_HAVE_sectionDisplayName:", name)
            ...
            with open("section_dates.json") as f:
                sect_dates = json.load(f)
            new_sect_dates = sect_dates.copy()
            ...
            for name in count:
                ...
                lru_sort.append([name, quantity])
                new_sect_dates[name] = str(date.today())

                if name not in sect_dates:
                    sect_dates[name] = "2022-01-01"  # if section is new, it makes it have the highest priority

            with open("section_dates.json", "w") as f:
                json.dump(new_sect_dates, f, indent=4)

            lru_sorted = sort_by_least_recently_used(quantity=lru_sort, dates=sect_dates)

            sections_f = open("sections.txt", "wb")
            if lru_sorted[-1] is True:
                sections_f.write(f"🛒👀 Tonight's #Fortnite Shop Sections:\n\n".encode("utf-8"))
            else:
                sections_f.write(f"🛒 Tonight's #Fortnite Shop Sections:\n\n".encode("utf-8"))

            for item in lru_sorted:
                if item not in [True, False]:  # doesn't add last item which is either True or False that tells us if any section hasn't been there for long or is new
            ...
            time_end = time.time()
            print(file_contents)
            tweepy_api.update_status(f"{file_contents[:280]}")
            print("Posted!")
            ...
            with open("Cache/cache1.json", "w") as file:
                json.dump(sections_data, file, indent=4)
...
def sort_by_least_recently_used(quantity, dates):
    """
    sections that appeared in the item shop least recently are placed on top,
    while those that appeared more recently are placed lower
    TODO: if section hasn't been in the shop for 6+ months, add clock emoji or smt at the end of the line or maybe even date?
    """
    sorted_sections = []
    sorted_by_date = dict(sorted(dates.items(), key=lambda date: datetime.strptime(date[1], "%Y-%m-%d")))
    caps_lock = False
    for sect in sorted_by_date:
        for quant_sect in quantity:
            if quant_sect[0] == sect:
                date_obj = datetime.strptime(sorted_by_date[sect], "%Y-%m-%d")
                reformatted_date = date_obj.date()
                if sorted_by_date[sect] == "2022-01-01":
                    final = f" • {sect} ({quant_sect[1]}x) 🆕"
                    caps_lock = True
                elif abs((datetime.today() - date_obj).days) >= 180:
                    caps_lock = True
                    final = f" • {sect} ({quant_sect[1]}x) | {reformatted_date.strftime('%d/%m/%y')}"
                else:
                    final = f" • {sect} ({quant_sect[1]}x)"
                sorted_sections.append(final)
    sorted_sections.append(caps_lock)
    return sorted_sections


# def sort_by_text_lenght_char_method(listx):
...
#                 with open(f"{cwd}unmeasured_characters.txt", "a") as file:
#                     file.write(f"\n{char}")
#
